...
prolific <- read.csv(file = "prolific_export_633701e0393f071b4684db1e.csv")
...
# Ensure 'prolific' is ordered by 'PROLIFIC_PID' in the same order as 'questionnaire'
prolific <- prolific[match(questionnaire$PROLIFIC_PID, prolific$Participant.id), ]
...
if(all(prolific$Participant.id == questionnaire$PROLIFIC_PID)){
  ...
  questionnaire$socio_timeTaken <- prolific$Time.taken
}
